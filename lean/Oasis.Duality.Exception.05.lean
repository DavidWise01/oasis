...
PER / CEPT / ION:
* PER  = observe at least once
* CEPT = locate the exception and test whether it belongs there
* ION  = act on the PER/CEPT result without rewriting it

Waldo rule:
* expected + exactly one found -> MATCH
* expected + none found        -> MISSING
* not expected + one found     -> UNEXPECTED
* more than one found          -> SHADOW / multiplicity

Seed walk:
  -0.5, +0.25a, +0.25b, -0.5, repeat

To avoid floating-point and Int-library normalization differences,
the seed carrier is an exact algebraic quarter-unit type:
  -0.5  = neg 2
  +0.25 = pos 1
...
theorem reserve_preserves (xs : List QuarterStep) :
    applyView .reserve xs = xs := by
  rfl

theorem backward_twice (xs : List QuarterStep) :
    applyView .backward (applyView .backward xs) = xs := by
  simp [applyView]

theorem upside_down_twice (xs : List QuarterStep) :
    applyView .upsideDown (applyView .upsideDown xs) = xs := by
  change (xs.map invert).map invert = xs
  induction xs with
  | nil =>
      rfl
  | cons x xs ih =>
...
theorem seed_walk_tt :
    seedWalk true true = [.neg 2, .pos 1, .pos 1, .neg 2] := by
  rfl

theorem seed_walk_tf :
    seedWalk true false = [.neg 2, .pos 1, .zero, .neg 2] := by
  rfl

theorem seed_walk_ft :
    seedWalk false true = [.neg 2, .zero, .pos 1, .neg 2] := by
  rfl
...
def dualityExceptionCheck : Bool :=
  (viewCount == 4) &&
  (seedWalk true true == [.neg 2, .pos 1, .pos 1, .neg 2]) &&
  (seedWalk true false == [.neg 2, .pos 1, .zero, .neg 2]) &&
