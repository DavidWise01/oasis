...
namespace Oasis.Duality.Exception04
...
theorem int_neg_neg_local (x : Int) :
    Int.neg (Int.neg x) = x := by
  cases x <;> rfl

theorem upside_down_twice (xs : List Int) :
    applyView .upsideDown (applyView .upsideDown xs) = xs := by
  change (xs.map Int.neg).map Int.neg = xs
  induction xs with
  | nil =>
      rfl
  | cons x xs ih =>
      simp only [List.map_cons]
      rw [int_neg_neg_local x, ih]
