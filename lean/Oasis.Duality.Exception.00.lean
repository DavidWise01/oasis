...
* if Waldo is not expected and exactly one is found -> UNEXPECTED
  ("what is Waldo doing there?")
* if more than one is found -> SHADOW / multiplicity branch

Seed walk:
  -0.5, +0.25a, +0.25b, -0.5, repeat

To keep this exact, positions are represented in quarter-units:
  -0.5  = -2
  +0.25 = +1

So:
  seedWalk a b = [-2, bit(a), bit(b), -2]

This is a symbolic/deterministic model.
-/

namespace Oasis.Duality.Exception
...
theorem four_views : viewCount = 4 := by rfl
theorem reserve_preserves (xs : List Int) : applyView .reserve xs = xs := by rfl
...
theorem seed_walk_tt : seedWalk true true = [-2, 1, 1, -2] := by rfl
theorem seed_walk_tf : seedWalk true false = [-2, 1, 0, -2] := by rfl
theorem seed_walk_ft : seedWalk false true = [-2, 0, 1, -2] := by rfl
theorem seed_walk_ff : seedWalk false false = [-2, 0, 0, -2] := by rfl
...
def checkAB (a b : Bool) : ABCheck := { a := a, b := b }
theorem check_a_preserved (a b : Bool) : (checkAB a b).a = a := by rfl
theorem check_b_preserved (a b : Bool) : (checkAB a b).b = b := by rfl
...
theorem expected_one_matches :
    classifyWaldo true [false, true, false] = .match := by decide
theorem expected_none_is_missing :
    classifyWaldo true [false, false, false] = .missingWaldo := by decide
theorem unexpected_one_reverses_question :
    classifyWaldo false [false, true, false] = .unexpectedWaldo := by decide
theorem two_waldos_are_shadow :
    classifyWaldo true [true, false, true] = .shadowWaldo 2 := by decide
theorem two_unexpected_waldos_are_shadow :
    classifyWaldo false [true, true, false] = .shadowWaldo 2 := by decide
theorem expected_clear_matches :
    classifyWaldo false [false, false, false] = .match := by decide
...
theorem ion_expected_proceeds : ion sampleExpected = .proceed := by decide
theorem ion_unexpected_investigates :
    ion sampleUnexpected = .investigateUnexpected := by decide
theorem ion_shadow_splits : ion sampleShadow = .splitShadow 2 := by decide
