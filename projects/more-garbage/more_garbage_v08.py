...
# --- v07 implicit structural geometry -----------------------------------

import hashlib as _hashlib

def _bits8(b: int) -> list[int]:
    return [(b >> shift) & 1 for shift in range(7, -1, -1)]

def holographic_like_for_like(positive: bytes, negative: bytes) -> bool:
    if len(positive) != len(negative):
        return False
    for a, b in zip(positive, negative):
        plus = _bits8(a)
        minus = list(reversed(_bits8(b)))
        for i in range(8):
            if plus[i] != minus[7 - i]:
                return False
    return True

def structural_geometry_verify(source_text: str, rebuilt_text: str) -> dict:
    a = source_text.encode("utf-8")
    b = rebuilt_text.encode("utf-8")
    phase_results = []

    for phase in range(1, 5):
        checks = []
        for recheck in range(1, 5):
            ok = holographic_like_for_like(a, b)
            checks.append(ok)
            if not ok:
                return {
                    "ok": False,
                    "phases": phase_results + [{"phase": phase, "rechecks": checks}],
                    "final_fifth": False,
                    "total_checks": sum(len(p["rechecks"]) for p in phase_results) + len(checks),
                }
        phase_results.append({"phase": phase, "rechecks": checks})

    final_fifth = holographic_like_for_like(a, b)
    return {
        "ok": final_fifth,
        "positive": "2^3",
        "negative": "-2^3",
        ...
        "final_fifth": final_fifth,
        "total_checks": 17,
    }

def recover_one_primitive_structural(
    source: str,
    index: int,
    max_window: int = 96,
    max_depth: int = 10,
    ...
    for width in range(1, limit + 1):
        ...
        for candidate in reversible_candidates(chunk, max_depth):
            ...
            rebuilt = inverse_path(normalized, candidate.path)
            if rebuilt is None:
                continue

            geometry = structural_geometry_verify(chunk, rebuilt)
            geometry_checks += geometry["total_checks"]
            if not geometry["ok"]:
                continue

            structural_contraction = (
                width > 1 and any(ord(ch) > 127 for ch in chunk)
            )
            if not (
                candidate.score < source_score
                or structural_contraction
            ):
                continue
    ...
    if not choices:
        return None, search_checks, geometry_checks
    ...
    return choices[0], search_checks, geometry_checks

def process_stream_structural(
    source: str,
    max_window: int = 96,
    max_depth: int = 10,
) -> dict:
    ...
    while i < len(source):
        ...
        recovered, checks, geom = recover_one_primitive_structural(
            source, i, max_window, max_depth
        )
        search_checks += checks
    ...
    return {
        "status": "DONE" if paused == 0 else "DONE_WITH_PAUSED",
        "input_sha256": _hashlib.sha256(source.encode("utf-8")).hexdigest(),
        "output": "".join(output),
        "search_checks": search_checks,
        "structural_geometry_checks": geometry_checks,
        "repaired_primitives": repaired,
        "paused_primitives": paused,
        "trace": trace,
    }
...
# --- v08 reversible orchestration ---------------------------------------

def build_reverse_map(forward_result: dict) -> list[dict]:
    repairs = [x for x in forward_result["trace"] if x.get("status") == "FOUND"]
    repairs.sort(key=lambda x: x["index"])
    mapped = []
    shrink = 0
    for item in repairs:
        output_index = item["index"] - shrink
        mapped.append({
            "source_index": item["index"],
            "output_index": output_index,
            "damaged_cluster": item["source"],
            "repaired_unit": item["output"],
            "width": item["width"],
            "path": list(item["path"]),
        })
        shrink += item["width"] - 1
...
def reverse_structural_stream(repaired_text: str, repair_map: list[dict]) -> dict:
    cursor = 0
    output = []
    operations = []
    structural_checks = 0
    for n, rec in enumerate(repair_map, start=1):
        output_index = rec["output_index"]
        if output_index < cursor:
            raise ValueError(f"overlapping reverse map at operation {n}")
        output.append(repaired_text[cursor:output_index])
        repaired_unit = repaired_text[output_index:output_index + 1]
        if repaired_unit != rec["repaired_unit"]:
            raise ValueError(f"repaired-unit mismatch at operation {n}")
        rebuilt_cluster = inverse_path(repaired_unit, tuple(rec["path"]))
        if rebuilt_cluster is None:
            raise ValueError(f"inverse path failed at operation {n}")
        geometry = structural_geometry_verify(rec["damaged_cluster"], rebuilt_cluster)
        structural_checks += geometry["total_checks"]
        if not geometry["ok"] or rebuilt_cluster != rec["damaged_cluster"]:
            raise ValueError(f"bit-for-bit reverse mismatch at operation {n}")
        output.append(rebuilt_cluster)
        operations.append({
            "op": n,
            "output_index": output_index,
            "source_index": rec["source_index"],
            "repaired_unit": repaired_unit,
            "inverse_path": list(rec["path"]),
            "rebuilt_cluster": rebuilt_cluster,
            "structural_checks": geometry["total_checks"],
            "bit_for_bit_match": True,
        })
        cursor = output_index + 1
    output.append(repaired_text[cursor:])
    return {
        "status": "DONE",
        "output": "".join(output),
        "operations": operations,
        "structural_geometry_checks": structural_checks,
    }

def roundtrip_structural(source: str, max_window: int = 96, max_depth: int = 10) -> dict:
    forward = process_stream_structural(source, max_window=max_window, max_depth=max_depth)
    repair_map = build_reverse_map(forward)
    reverse = reverse_structural_stream(forward["output"], repair_map)
    source_hash = _hashlib.sha256(source.encode("utf-8")).hexdigest()
    repaired_hash = _hashlib.sha256(forward["output"].encode("utf-8")).hexdigest()
    rebuilt_hash = _hashlib.sha256(reverse["output"].encode("utf-8")).hexdigest()
    return {
        "status": "PASS" if reverse["output"] == source else "FAIL",
        "forward": {
            "status": forward["status"],
            "repaired_primitives": forward["repaired_primitives"],
            "paused_primitives": forward["paused_primitives"],
            "search_checks": forward["search_checks"],
            "structural_geometry_checks": forward["structural_geometry_checks"],
            "source_sha256": source_hash,
            "repaired_sha256": repaired_hash,
        },
        "reverse": {
            "operations": len(reverse["operations"]),
            "structural_geometry_checks": reverse["structural_geometry_checks"],
            "rebuilt_sha256": rebuilt_hash,
            "matches_source": reverse["output"] == source,
        },
        "geometry": {
            "positive": "2^3",
            "negative": "-2^3",
